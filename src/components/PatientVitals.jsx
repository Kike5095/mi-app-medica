...
const PatientVitals = ({ patient, onBack }) => {
  ...
  const handleAddVital = async (e) => {
    ...
    try {
      // --- LÓGICA DE CONVERSIÓN A NÚMERO ---
      // Reemplazamos la coma (,) por un punto (.) y lo convertimos a número flotante
      const tempAsNumber = parseFloat(temperature.replace(',', '.'));
      const heartRateAsNumber = parseFloat(heartRate.replace(',', '.'));
      const respiratoryRateAsNumber = parseFloat(respiratoryRate.replace(',', '.'));
      const saturationAsNumber = parseFloat(saturation.replace(',', '.'));

      await addDoc(collection(db, "patients", patient.docId, "vitals"), {
        temperature: tempAsNumber,
        bloodPressure: bloodPressure, // Este se queda como texto
        heartRate: heartRateAsNumber,
        respiratoryRate: respiratoryRateAsNumber,
        saturation: saturationAsNumber,
        note: note,
  ...
  const handleFinalizeTreatment = async () => { /* ... (código sin cambios) ... */ };

  // ... (El resto del código de gráficas, estilos y JSX es el mismo)
  // Lo incluyo todo para que simplemente copies y pegues
  const FinalPatientVitals = ({ patient, onBack }) => {
    // ... (código existente completo)
  };
  return <FinalPatientVitals patient={patient} onBack={onBack} />;
};

// --- CÓDIGO COMPLETO Y FINAL PARA REEMPLAZAR EN PatientVitals.jsx ---
const FullPatientVitals = ({ patient, onBack }) => {
    const [temperature, setTemperature] = useState('');
    const [bloodPressure, setBloodPressure] = useState('');
    const [heartRate, setHeartRate] = useState('');
    const [respiratoryRate, setRespiratoryRate] = useState('');
    ...
    const styles = {
        container: { padding: '20px', fontFamily: 'sans-serif' }, header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' }, headerActions: { display: 'flex', alignItems: 'center', gap: '15px' }, backButton: { padding: '8px 15px', border: '1px solid #ccc', borderRadius: '5px', cursor: 'pointer' }, finalizeButton: { padding: '8px 15px', backgroundColor: '#ffc107', color: 'black', border: 'none', borderRadius: '5px', cursor: 'pointer' }, form: { padding: '20px', backgroundColor: '#f9f9f9', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)', marginTop: '20px' }, formGroup: { marginBottom: '15px' }, label: { display: 'block', marginBottom: '5px', fontWeight: 'bold' }, input: { width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }, textarea: { width: '100%', minHeight: '80px', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', fontFamily: 'sans-serif', fontSize: '1rem' }, button: { padding: '10px 20px', backgroundColor: '#4CAF50', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }, table: { width: '100%', borderCollapse: 'collapse', marginTop: '30px' }, th: { border: '1px solid #ddd', padding: '8px', backgroundColor: '#f2f2f2', textAlign: 'left' }, td: { border: '1px solid #ddd', padding: '8px' }, chartsContainer: { marginTop: '40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '20px' },
        section: { marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #ccc' }
    };

    return (
        <div style={styles.container}>
            <header style={styles.header}><div style={styles.headerActions}><button onClick={onBack} style={styles.backButton}>&larr; Volver a la lista</button>{patient.status === 'Activo' && (<button onClick={handleFinalizeTreatment} style={styles.finalizeButton}> Finalizar Tratamiento </button>)}</div><h1>Historial de: {patient.name}</h1></header>
            <hr />
            <div style={styles.section}>
                <form style={styles.form} onSubmit={handleAddVital}>
                    <h3>Añadir Signos Vitales y Nota</h3>
                    <div style={styles.formGroup}><label style={styles.label}>Frecuencia Cardíaca (lpm)</label><input type="text" style={styles.input} value={heartRate} onChange={(e) => setHeartRate(e.target.value)} /></div>
                    <div style={styles.formGroup}><label style={styles.label}>Frecuencia Respiratoria (rpm)</label><input type="text" style={styles.input} value={respiratoryRate} onChange={(e) => setRespiratoryRate(e.target.value)} /></div>
                    ...
                    <div style={styles.formGroup}><label style={styles.label}>Saturación (%)</label><input type="text" style={styles.input} value={saturation} onChange={(e) => setSaturation(e.target.value)} /></div>
                    <div style={styles.formGroup}><label style={styles.label}>Temperatura (°C)</label><input type="text" style={styles.input} value={temperature} onChange={(e) => setTemperature(e.target.value)} /></div>
...
return <FullPatientVitals patient={patient} onBack={onBack} />;